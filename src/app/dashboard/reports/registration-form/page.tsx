            ...
            <div className="w-full overflow-x-auto pb-8 scrollbar-thin bg-muted/20 rounded-xl p-2">
                <div className="min-w-[600px] flex justify-center">
                    <div className="mx-auto bg-white text-black border shadow-2xl flex flex-col" id="printable-form" style={{ width: '600px' }}>
                        <header className="text-center space-y-4 p-6 border-b-4 border-slate-900">
                            ...
                            <div className="flex flex-col items-center gap-4">
                                ...
                                <div className="flex flex-col items-center gap-3">
                                    <Avatar className="h-16 w-16 border-2 border-primary/20 bg-white shadow-sm p-1">
                                        <AvatarImage src={clubLogoUrl || undefined} alt="Club Logo" className="object-contain" />
                                        <AvatarFallback className="bg-primary text-white text-2xl font-black">{clubInitial}</AvatarFallback>
                                    </Avatar>
                                    <div className="space-y-0.5">
                                        <h1 className="text-xl font-black uppercase tracking-tight">
                                            {formType === 'checklist' ? 'PIÈCES À FOURNIR POUR LE DOSSIER' : `FICHE D'INSCRIPTION ${formType === 'adult' ? 'ADULTE' : 'JUNIOR'}`}
                                        </h1>
                                        <p className="text-primary font-black tracking-[0.4em] uppercase text-sm italic">{clubName}</p>
                                    </div>
                                </div>
                            )}
                            <div className="flex items-center justify-center font-bold text-xs bg-slate-50 py-2 rounded-lg border border-dashed border-slate-300">
                                <span className="px-4 font-black uppercase tracking-tighter">SAISON SPORTIVE : 20 . . . / 20 . . .</span>
                            </div>
                        </header>

                        <div className="p-8 space-y-8 text-left flex flex-col h-full min-h-[700px]">
                            {formType === 'checklist' ? (
                                <div className="space-y-8 py-4">
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-3 mb-6">
                                            <ClipboardList className="h-6 w-6 text-primary" />
                                            <h3 className="font-black text-lg uppercase tracking-tight italic">LISTE DES DOCUMENTS OBLIGATOIRES</h3>
                                        </div>
                                        <ul className="grid grid-cols-1 gap-4">
                                            {[
                                                ...
                                                <li key={i} className="flex items-start gap-4 p-4 border-2 rounded-xl bg-slate-50 shadow-sm">
                                                    <div className="h-6 w-6 border-2 border-primary rounded-lg shrink-0 mt-0.5 flex items-center justify-center font-black text-primary text-xs italic">!</div>
                                                    <div>
                                                        <p className="font-black text-sm uppercase tracking-tight text-slate-900">{item.t}</p>
                                                        <p className="text-[10px] text-muted-foreground font-semibold italic">{item.d}</p>
                                                    </div>
                                                </li>
                                            ))}
                                        </ul>
                                ...
                                <>
                                    <div className="space-y-4">
                                        <h3 className="font-black text-[10px] uppercase tracking-[0.2em] bg-slate-900 text-white px-4 py-2 w-fit rounded shadow-md italic">I. IDENTITÉ DU JOUEUR</h3>
                                    ...
                                    {formType === 'junior' && (
                                        <div className="space-y-4 pt-2">
                                            <h3 className="font-black text-[10px] uppercase tracking-[0.2em] bg-slate-900 text-white px-4 py-2 w-fit rounded shadow-md italic">II. RESPONSABLE LÉGAL (TUTEUR)</h3>
                                            <div className="space-y-4 text-xs font-medium">
                                                <div className="flex items-center"><div>NOM ET PRÉNOM :</div><DottedLine /></div>
                                                <div className="grid grid-cols-2 gap-x-8">
                                    ...
                                    <div className="space-y-4 pt-2 flex-grow flex flex-col">
                                        <h3 className="font-black text-[10px] uppercase tracking-[0.2em] bg-slate-900 text-white px-4 py-2 w-fit rounded shadow-md italic">III. AUTORISATION ET DÉCLARATION</h3>
                                        <div className="space-y-4 text-xs leading-relaxed italic text-slate-700">
                                            ...
                                            <p className="font-black text-black border-l-4 border-primary pl-4 not-italic py-1 bg-primary/5 uppercase tracking-tighter text-[9px]">AVIS IMPORTANT : Cette fiche d'inscription doit impérativement être légalisée auprès des autorités compétentes pour être valide.</p>
                                        </div>
                                    </div>
                                </>
                            )}
                                ...
                                <div className="w-full flex flex-row justify-between items-start gap-8">
                                    <div className="space-y-4 pt-2 text-left">
                                        <div className="flex items-center">
                                            <span className="font-bold text-sm">Fait à</span><div className="w-32 border-b border-dotted border-gray-400 mx-2"></div>
                                        </div>
                                        <div className="flex items-center">
                                            <span className="font-bold text-sm">Le</span><div className="w-32 border-b border-dotted border-gray-400 mx-2"></div>
                                        </div>
                                    </div>
                                    <div className="flex flex-col items-center gap-2">
                                        <div className="text-center w-48 h-28 border-2 border-slate-200 rounded-xl p-4 flex flex-col items-center bg-slate-50/50 shadow-inner">
                                            <p className="text-[8px] font-black uppercase tracking-[0.2em] text-slate-400 mb-auto">Cachet du Club & Signature</p>
                                            <div className="w-36 border-b border-slate-200 mb-2"></div>
                                        </div>
                        ...
                        <footer className="p-6 bg-slate-900 text-white flex justify-between items-center mt-auto border-t-2 border-primary">
                            <p className="text-[8px] font-black uppercase tracking-[0.2em] opacity-50">© {new Date().getFullYear()} {clubName} - ADMINISTRATION SPORTIVE</p>
                            <div className="flex items-center gap-1.5 text-primary font-black uppercase tracking-widest italic text-[8px]">
