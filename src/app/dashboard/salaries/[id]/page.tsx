...
export default function SalaryDetailPage(props: { params: Promise<{ id: string }> }) {
  ...
  const totalPaid = salary.transactions?.reduce((sum: number, t: any) => sum + t.amount, 0) || 0;

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => router.back()}><ArrowLeft className="h-6 w-6" /></Button>
        <h1 className="text-3xl font-bold tracking-tight">Détails de Salaire</h1>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><FileText /> {salary.description}</CardTitle>
